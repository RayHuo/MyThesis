...
\begin{proof}
	%%% 充分性
	$(\Rigtharrow) \space$ 即证明若$S^*$为$tr_p(ct_U(P),U)$的回答集，且$S \backslash U = S^* \cap Atoms(P)$，则原子集$S$为$P$的回答集。
	
	根据$ct_U(P)$的定义，其第一部分。其考虑了程序结论$U$中满足$R^-(E,P) \subseteq in'_U(P)$的所有非空子集$E$。
	
	
	%%% 必要性
	$(\Leftarrow) \space$ 即证明若$S$为$P$的回答集，则存在$S^*$为为$tr_p(ct_U(P),U)$的回答集，且满足$S \backslash U = S^* \cap Atoms(P)$。
