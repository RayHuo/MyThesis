...
程序结论的原始定义是面向文字集的，而在只考虑正文字的情况下，文字集可以被视为原子集，下面所涉及的程序结论的使用都将其视为一个原子集。此外，程序结论并不需要在求解出逻辑程序的所有回答集后才能得到，第二章已经给出了相关计算方法\ref{conq_P}。这样也保证了本文使用程序结论的可行性。接下来给出一个计算程序结论的例子。
% 插入算法
%\RestyleAlgo{ruled}\LinesNumbered\input{algorithm/Consequence.tex}
