...
\begin{algorithm}
  ...
  %\For{$i\leftarrow 2$ \KwTo $l$}{
    ...
    \Switch(\tcp*[h]{公式类型}){$\varphi.type$}
    {
        ...
        \Case(\tcp*[h]{根据规则(N6)可得}){$\rightarrow$}
        {
            \If(\tcp*[h]{右子公式为否定式}){IsNegative($\varphi.right$)}
            {
        ...
        \Case(\tcp*[h]{省略原子、合取、全称、存在量词公式的情形}){$\neg$}
        {
            \Switch(\tcp*[h]{公式类型}){$\varphi.left.type$}
            {
                \Case{$\neg$}
                {
