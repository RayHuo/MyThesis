...
\begin{definition}[文字]\label{literal}
...
在定义\ref{proposition_logic}中的连接词的作用是把若干个文字连接成命题公式。每个连接词都有自己的真值表，可以概括如下\cite{luger2003AI}：
...
\begin{definition}[单位子句（$Unit~Clause$）] \label{unit_clause} \upcite{zhang1996efficient}
...
其中$UP(L,P)~=~L \cup UP(assign(L,P))$，$UP(assign(L,P))$为定义\ref{C_S_U_P}中的单元传播；$GUS(P,L)$为定义\ref{C_S_U_P}中的最大无理集（$Great~Unfounded~Set$）；$lfp$为计算极小不动点的算子。
...
\begin{definition}[环\cite{lin2004assat}]\label{loop}
...
\begin{definition}[环公式\upcite{lin2004assat}]\label{loop_formula}
