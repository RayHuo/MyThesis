...
在定义\ref{proposition_logic}中的连接词的作用是把若干个文字连接成命题公式。
...
经典逻辑程序只能推出正文字的真值，而实际情况下，负文字的真值也需要被确定。
...
其中$n \geq m \geq k \geq 1$，$a_i$为原子(atom)。在$k$，$m$，$n$取不同范围的值时，形式$(\ref{asp_rule})$所表示的规则有不同的意义，具体如下\upcite{lin2004assat}：
\begin{itemize}
...
稳定模型语义解决了非单调推理无法解释失败即否定的问题，此外还给出了一个规约方法(G-L规约)，以化简一个ASP逻辑程序中的失败即否定。
...
Gelder和Ross等(1991)提出了逻辑程序的良序模型(Well-founded Model)语义\upcite{van1991well}，良序模型需要通过ASP逻辑程序的最大无理集(Greatest Unfounded Set)来计算。
...
Gelder和Ross等(1991)证明了两个无理集的并集仍然是一个无理集，因此ASP逻辑程序$P$基于文字集$I$存在一个最大无理集(Greatest Unfounded Set)，记为$GUS(P,L)$。定义以下算子以计算$GUS(P,L)$：
...
接下来将说明ASP逻辑程序中的一个重要性质，即ASP逻辑程序中的环以及对应的环公式。
...
基于这样的事实，Lin和Zhao(2004)提出了使用环公式求解ASP逻辑程序回答集的方法，并实现了ASP求解器——ASSAT。

Lin和Zhao(2004)扩展了完备的定义，并给出如下定义：
\begin{definition}[完备(Completion) \upcite{lin2004assat}]\label{completion}
	给定ASP逻辑程序$P$，其完备，记为$Comp(P)$。$Comp(P)$是以下规则的集合：
	\begin{itemize}
...
直观地，分割集中的原子对应为ASP逻辑程序$P$的正依赖图中出度为零的顶点\upcite{ji2010phd}。基于分割集划分而成的$bottom$和$top$两部分的具体定义如下：
...
Lifschitz和Turner定义了操作$e_U(P,X)$来联合$bottom$的回答集和$top$求出另一部分的回答集。
...
接下来将介绍本文提出的新分割集和新程序分割方法，以及通过具体的应用场景来体现新分割集的意义。
