...
\section{命题逻辑}
命题(Proposition)是非真即假的陈述句。我们一般使用字母代表一个命题。命题逻辑由命题公式和一套推理规则所组成，其中命题公式和推理规则就是命题逻辑的运算本体和运算规则\upcite{zlw1989sllj}。
...
然后，本节引入环公式的定义。
\begin{definition}[环公式(Loop Formula) \upcite{lin2002assat}]\label{loop_formula}
...
\begin{definition} \label{asp_solution}
	给定ASP逻辑程序$P$及其分割集$U$，$P$关于$U$的一个方案，是一个原子集组合$\langle X,Y \rangle$，具体地：
	\begin{itemize}
		\item $X$是$b_U(P)$的回答集；
