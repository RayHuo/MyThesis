...
本章涉及两个实验：程序分割实验和程序化简实验。程序分割实验中首先直接求解逻辑程序的回答集，然后使用新程序分割方法求解逻辑程序回答集，并比较两种方法的效率。程序化简实验中则对比原逻辑程序和进行程序化简后的逻辑程序的求解效率。
...
本节给出程序分割实验和程序化简实验所用到的实验工具和测例内容。

% lex and yacc
\subsection{实验工具}

本章的两个实验的实验工具是一致的。在词法分析和语法分析上都分别使用$flex$和$bison$，在例化和求解方面都使用$gringo$和$clasp$。以下来具体介绍这几个工具。
