...
本章涉及两个实验：程序分割实验和程序化简实验。程序分割实验中首先直接求解逻辑程序的回答集，然后使用新程序分割方法求解逻辑程序回答集，并比较两种方法的效率。程序化简实验中则对比原逻辑程序和进行程序化简后的逻辑程序的求解效率。本文使用词法分析器$flex$和语法分析器$bison$对ASP逻辑程序进行解析，通过C/C++编写新程序分割和程序化简部分的功能并输出操作后的ASP逻辑程序，最后使用$gringo$对操作后的ASP逻辑程序进行例化，使用$clasp$求解ASP逻辑程序。

\section{实验工具}

本文的两个实验所使用的辅助工具一致。在词法分析和语法分析上都分别使用$flex$和$bison$，在例化和求解方面都使用$gringo$和$clasp$。以下来具体介绍这几个工具。
...
有了新程序分割方法及新的分割理论后，我们知道有两种方法可以求解逻辑程序$P$：
\begin{itemize}
	\item 通过分割集$U$来把逻辑程序$P$划分及构造为两部分：$b_U(P) \cup EC_U(P)$和\\
	$e_U(t_U(P,X),X) \cup ECC_U(P,X)$，并通过这两部分的回答集$X$和$Y$来求解得到$P$的回答集；
	\item 直接求解逻辑程序$P$。
\end{itemize}
...
对于分割集$U$的选择方面，本实验使用整个右子图中的节点对应的原子组成分割集$U$，基于哈密顿回路要求经过图中的所有节点，故这部分原子其实都能被ASP逻辑程序$P$的每个回答集所满足。此外，这样的分割集$U$能保证$|Atoms(b_U(P)) \backslash U| = 3$。这两点性质跟第3章中分析底部计算复杂性时提出的提速方案一致。
...
\begin{enumerate}\label{splitting_experiment_process}
	...
	\item 计算出分割集$U$；
	\item 基于分割集$U$对例化后的ASP逻辑程序使用新程序分割方法进行划分，得到$bottom$（即$b_U(P) \cup EC_U(P)$）和$top$（即$e_U(dt_U(P,X),X) \cup ECC_U(P,X)$）两部分逻辑程序；
...
具体的实验环境参数如表\ref{tab_environment}所示。
\\
\begin{table}[htb]
...
本实验中使用的珍珠图包含2颗珍珠，每个珍珠的基的范围是10到50，同一个珍珠图里的每颗珍珠的基是相等的。对于每个测例，根据其对应珍珠图中包含的珍珠数量$k$和每个珍珠的基$N$，将该测例记为$k-N$，如：一个例化后的ASP逻辑程序对应的珍珠图里包含2颗珍珠，每颗珍珠的基为10，则将此测例记为“2-10”。

本实验对每个测例执行上述的操作过程，并将统计得到的求解耗时数据整理如表\ref{s_time_compare}所示。
\begin{table}[htb]
	\caption{两种求解方法的求解耗时对比}\label{s_time_compare}
	\begin{center}
...
表\ref{s_time_compare}中的“Benchmark”一列记录的是各个测例的记号，“bottom”一列记录的是求解$b_U(P) \cup EC_U(P)$部分的耗时，“top”一列记录的是求解$e_U(dt_U(P,X),X) \cup ECC_U(P,X)$部分的耗时，“bottom+top”一列记录的是“bottom”和“top”两部分的耗时总和，“whole”一列记录的是直接求解原ASP逻辑程序的耗时，“match”一列记录的是两种方法求解得到的回答集的内容是否一致。上述的时间单位均为秒。
...
本实验通过比较两种方法求解得到的回答集内容的一致性，验证了使用新程序分割方法求解ASP逻辑程序的正确性。此外，本实验通过表\ref{s_time_compare}中的数据验证了使用新程序分割方法求解ASP逻辑成的有效性。
这些结果验证了本节开始时所给出的两个假设的正确性。
...
本实验所用到的测例为题依旧是哈密顿回路问题，并继续使用Niemel{\"a}在1999年对该问题的编码作为例化前的ASP逻辑程序。用于例化ASP逻辑程序的事实文件也与程序分割实验的一致，为“2-N”结构的珍珠图。使用$gringo$基于事实文件例化ASP逻辑程序后的输出文件，例化后的ASP逻辑程序，即为本实验的测例。

本实验对每个测例具体的实验步骤为：
\begin{enumerate} \label{simplifying_experiment_process}
	\item 直接使用$clasp$求解例化后的ASP逻辑程序，统计其平均求解时间为$t_1$；
	\item 通过公式\ref{conq_P}计算出例化后的ASP逻辑程序的程序结论$U$；
	\item 使用程序结论$U$对例化后的ASP逻辑程序执行程序化简操作，得到新的ASP逻辑程序；
	\item 使用$clasp$新的ASP逻辑程序进行求解，并统计其平均求解时间为$t_2$。
\end{enumerate}
...
此外，统计平均值的定义与程序分割实验中的一致，本实验的实验环境也与程序分割实验中的一致。程序化简的操作通过C/C++编码实现，输出化简后的ASP逻辑程序。求解操作都直接使用$clasp$完成。


\subsection{程序化简实验数据分析}

本实验使用的珍珠图与程序分割实验中的一致，一共统计了9个测例的实验数据。对每个测例执行实验步骤\ref{simplifying_experiment_process}，并统计进行程序化简前后的其平均求解时间，同时对比使用程序化简方法得到的回答集与原程序的回答集的内容是否一致。具体的数据整理如表\ref{sim_time_compare}。
\begin{table}[htb]
	\caption{程序化简前后的求解耗时对比}\label{sim_time_compare}
	\begin{center}
		\begin{tabular}{c|c|c|c}
			...
			2-25 & 5.56 & 1.76 & true	\\
			\hline
			2-30 & 14.07 & 5.12 & true	\\
			\hline
			2-35 & 27.16 & 8.55 & true	\\ 
			\hline
			2-40 & 52.53 & 16.03 & true	\\
			\hline
			2-45 & 106.93 & 38.42 & true \\
			\hline
			2-50 & 171.94 & 62.35 & true \\
			\hline
		\end{tabular}
	\end{center}
\end{table}

表\ref{sim_time_compare}中的“Benchmark”一列记录的是各个测例的记号，“original”一列记录的是各个测例直接使用$clasp$求解回答集的平均耗时，“simplifying”一列记录的是进行程序化简后的ASP逻辑程序的求解回答集的平均耗时，“match”一列记录的是两种方法求解得到的回答集的内容是否一致。上述的时间单位均为秒。

对于表\ref{sim_time_compare}提供的数据，本节可以归纳得出以下的观察结果：
\begin{itemize}
	\item 求解程序化简后的ASP逻辑程序得到的回答集联合程序结论$U$得到的回答集与直接求解原程序的回答集内容一致；
	\item 基于程序结论使用程序化简方法求解ASP逻辑程序的耗时比直接求解ASP逻辑程序要明显的少；
	\item 基于程序结论使用程序化简方法求解ASP逻辑程序的效率随着程序规模的增长稳定在直接求解ASP逻辑程序3到4倍之间。 
\end{itemize}

通过上述的观察结果可以知道，本实验开始时给出的两个假设成立。观察结果中的第一点验证了使用程序结论对ASP逻辑程序进行程序化简的正确性。观察结果中的第二点从实验层面给予了基于程序结论的程序化简方法的有效性一个正面的支持。
