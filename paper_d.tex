...
% 去掉参数openany与oneside，用以实现论文的每一个章节从奇数页开始打印
...
\begin{CJK*}{GBK}{}
	...
	{\item[　　\hskip\labelsep \theorem@headerfont ##1\ ##2]}%      %把上面两句中的  \theorem@separator   删掉后，定义XX、证明XX 等就不会在XX后面出现冒号！！！
	{\item[　　\hskip\labelsep \theorem@headerfont ##1\ ##2\ (##3)]}
	\makeatother
	\theoremstyle{myplain}
	\theoremheaderfont{\CJKfamily{hei}\rmfamily}  %%%%%%%%%%% old
	\theorembodyfont{\CJKfamily{kai}\rmfamily}	 %%%%%%%%%%% old
%	\theoremheaderfont{\CJKfamily{song}\rmfamily} %%%%%%%%%%%% new add
%	\theorembodyfont{\CJKfamily{song}\rmfamily}	 %%%%%%%%%%%% new add
	\theoremseparator{：}
	\newtheorem{definition}{\hei 定义}[chapter]   %%%%%%%%%%% old
	\newtheorem{proposition}{\hei 命题}[chapter]  %%%%%%%%%%% old
%	\newtheorem{definition}{\song 定义}[chapter]   %%%%%%%%%%% new add
%	\newtheorem{proposition}{\song 命题}[chapter]  %%%%%%%%%%% new add
	%\newtheorem{lemma}[definition]{\hei 引理}
	\newtheorem{lemma}{\hei 引理}[chapter]    %若引理单独从 X.1 开始的话，就使用这一句，否则使用上面一句！！                   %%%%%%%%%%%% old
%	\newtheorem{lemma}{\song 引理}[chapter]  %%%%%%%%%%%% new add
	\newtheorem{theorem}{\hei 定理}[chapter]	%%%%%%%%%%%% old
%	\newtheorem{theorem}{\song 定理}[chapter]  %%%%%%%%%%%% new add
	\newtheorem{axiom}{\hei 公理}
	\newtheorem{corollary}{\hei 推论}[chapter] %%%%%%%%%%%% old
%	\newtheorem{corollary}{\song 推论}[chapter]  %%%%%%%%%%%% new add
	%\theorembodyfont{\CJKfamily{song}\rmfamily} % body部分为书宋体
	\newtheorem{exercise}[definition]{\hei 练习}
	\newtheorem{example}{\hei 例}[chapter]		%%%%%%%%%%%% old
%	\newtheorem{example}{\song 例}[chapter]		%%%%%%%%%%%% new add
...
%	\theorembodyfont{\CJKfamily{song}\rmfamily}    %%%%%%%%%%%% new add
	...
	\newtheorem{proof}{\hei 证明}		%%%%%%%%%%%% old
%	\newtheorem{proof}{\song 证明}		%%%%%%%%%%%% new add
...
\usepackage[ps2pdf,
% pdfstartview=FitH,
CJKbookmarks=true, colorlinks=true, linkcolor=black, citecolor=blue]{hyperref}
\AtBeginDvi{\special{pdf:tounicode GBK-EUC-UCS2}} % GBK -> Unicode
\else
\usepackage[pdftex]{graphicx}
...
\newcommand{\papername}{一种改进的回答集逻辑程序分割方法及程序化简研究}
\newcommand{\schoolname}{中山大学硕士学位论文}
...
%		\include{additional}			%% new add
%		\thispagestyle{empty}		%% new add
