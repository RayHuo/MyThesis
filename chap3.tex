...
被分割集$U$破坏的环的环公式可能不能为$bottom$的回答集$X$所满足，所以需要剩下部分的回答集联合$X$一起去满足。定义$SL_U(P,X)$就是为了找出ASP逻辑程序$P$中被$U$破坏且环公式不能被$X$所满足的环。根据这些被破坏的环的特点，引用Lin和Zhao(2002)提出的环理论，本节定义新的$top$的关键部分以保证最后得到的回答集联合$X$能满足这部分环的环公式，进而保证ASP逻辑程序$P$中所有环的环公式都能被满足。
...
称$e_U(t_U(P,X),X) \cup ECC_U(P,X)$为新的$top$，根据新的$top$的作用，可以得到以下命题。
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{proposition} \label{po_XY_satisfy_allLoop}
	给定ASP逻辑程序$P$及其分割集$U$，一组原子集$\langle X,Y \rangle$是$P$基于$U$的一组方案，则$(X \cup Y) \cap Atoms(P)$能满足$P$中的所有环的环公式。
\end{proposition}
...
下面给出一个计算ASP逻辑程序$P$基于其分割集$U$的方案的例子。
\begin{example}
