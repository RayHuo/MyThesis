...
Lifschitz和Turner的程序分割方法是基于分割集进行的，而根据\ref{section_3_1_2}的结果表明，对于大部分逻辑程序，分割集往往就是$Atoms(P)$。这样的分割集无法划分逻辑程序，所以本文提出了新的程序分割方法，以支持分割集可以为任意原子集。
首先，本文继续使用定义\ref{bottom_top}中的$b_U(P)$和定义\ref{e_U_P_X}中的操作$e_U(P,X)$，依旧记分割集为U。而此时分割集$U$并不存在结构上的独立性，即：
\begin{equation}
	Atoms(b_U(P)) \subseteq U
\end{equation}
