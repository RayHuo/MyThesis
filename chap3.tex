...
其中$x_{E_i}$为基于$SL_U(P,X)$中的$smei-loop$引入的新原子，对于这些新原子，最后通过逻辑交$Atoms(P)$即可消去。新的$top$由三部分组成，其中第一部分从$P$中去除了$bottom$部分，同时先把$out_U(P)$，事实上$out_U(P)$不是单纯地被去掉，而是在第三部分被重构以与第二部分基于$in_U(P)$的规则构成环。下面给出新的$top$求解得到的回答集$Y$能满足$body(r)$，其中$r \in R^-(L,P)$的证明。
\begin{proof}
	...
	接着证明$t_U(P)$中的第二和第三部分能够构造出关于$L \backslash U$的环。
	
	
	接着证明$(X \cup Y) \cap Atoms(P)$满足$P$（根据命题\ref{satisfy_proposition}）。
\end{proof}
