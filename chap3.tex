...
本节以正规逻辑程序在程序分割方法上的计算复杂性为例进行分析。将先给出程序分割方法各个部分的规则或原子集合的计算时间复杂度。
然后分析分割集如何影响程序分割过程的计算复杂性，并指出符合哪些条件的原子集作为分割集才会让程序分割更高效。

\subsection{分割方法的计算时间复杂度}
程序分割方法涉及多个规则或原子集合的计算，可以把这些规则或原子集合分为与$bottom$和$top$相关的两部分进行分析，并得到以下的结果：
...
可以看到这些规则或原子集合除了$DSL_U(P,X)$外，都可以在多项式时间内得到，故这部分的耗时基本不会对整体计算造成影响。而对于$DSL_U(P,X)$的时间复杂度则主要受到$C=|head(in_U(P)) \cup body^+(out_U(P))|$的影响，而这部分事实上可以通过有效选择$U$的内容而使$O(M*2^C)$维持在一个较小的数值下。直观来说就是可以通过找出合适的$U$来使得被破坏且环公式不能被底部回答集$X$所满足的环尽量地少。
...
本小节对第一个问题进行分析。$bottom$中的$EC_U(P)$是为了补充$Atoms(b_U(P)) \\ \backslash U$中的原子在回答集中的真值可能性而定义的，就是这种真值可能性的遍历行为导致了回答集的指数增多。
