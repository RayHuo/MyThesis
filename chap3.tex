...
本章将对Lifschitz和Turner提出的分割集和程序分割理论进行介绍。
首先给出了其程序分割方法的正确性证明，及提出了计算其定义的分割集的算法，然后分析了其分割集的局限性。
接着，本章提出了新分割集和新程序分割方法。
实际上是提出了一个可以对任意原子集作为分割集都有效的程序分割方法。
...
Lifschitz和Turner在1994年的原文中只给出了程序分割方法的定义，即根据定义(\ref{asp_solution})分别求出底部$b_U(P)$和化简后的顶部$e_U(t_U(P),X)$的回答集$X$和$Y$，根据方案$\langle X,Y \langle$得到原程序的回答集。
