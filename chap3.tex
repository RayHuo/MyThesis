\chapter{一阶限定理论到稳定模型语义的翻译}

本章主要介绍求解器实现的理论部分，基于对一阶限定理论的研究，阐明了一阶限定理论和一阶稳定模型语义的关系，提出了在一般结构下从一阶限定理论到稳定模型语义的翻译，构成了求解器的核心算法。本章将分别介绍并行限定理论和优先级限定理论的翻译方法，并提出了翻译在有穷结构下优化的方法。


\section{并行限定理论的翻译}

章衡在论文\cite{zhang2011translating}中已经介绍了一阶限定理论到一阶稳定模型语义的一个等价翻译，但是该翻译中没有处理可变谓词的情况，即只能解决无可变谓词常元的限定理论。在本节中，我们提出了一般结构下，并行限定和优先级限定到到稳定模型语义的翻译。

%\subsection{并行限定理论到稳定语义的翻译}

本小节主要关注于并行限定理论的翻译。对比并行限定的定义公式(\ref{form_defcirc})与稳定理论的定义公式(\ref{form_defsm})，它们主要的区别在于二阶部分的可变谓词部分，然而这些可变谓词$\sigma^*_{v}$并不参与谓词集的比较，翻译的主要困难在于不影响$\sigma^*_{i}<\sigma_i$和$\varphi$的情况下，不通过稳定模型语义下的比较而消去它们。因此，对于所有谓词$Q\in\sigma_v$，翻译算法引入与其有相同元数的辅助谓词常元$Q'$，如果使$Q'$为真的话，$Q'^*$在二阶公式中模拟了谓词变元$Q^*$。本文得到如下的翻译方法。

\begin{definition}\label{def_parallelcirc2sm}
设$\varphi$为任意具有否定范式的一阶语句，$\sigma_i$和$\sigma_v$表示两个谓词常元集合，定义
...
设$\varphi$为具有否定范式的任意一阶语句，$\sigma_i$和$\sigma_v$是连个谓词常元集合，则$\exists \sigma'_v \exists \gamma \mathbf{SM}[Tr_v(\varphi;\sigma_i;\sigma_v);\sigma_i;\sigma'_v,\gamma]$ 等价于$\mathbf{CIRC}[\varphi;\sigma_i;\sigma_v]$，其中$\sigma'_v$是相对于$\sigma_v$的辅助谓词常元集合。
\end{proposition}

根据并行限定定义的二阶公式(\ref{form_defcirc})和稳定模型语义的定义，对于上述定义和命题有如下的解释：
\begin{itemize}
  \item 公式$\varphi$为NNF的形式的意义在于，经典一阶下公式的等价性在限定理论中仍然适用，所以对于任意的一阶公式，在限定理论中都存在一个与之等价的否定范式，因此可以事先将其转成NNF，方便处理；
  \item $\varphi^{\neg\neg}$的意义在于$\mathrm{St}(\varphi^{\neg\neg})$等价于$\varphi$；
  \item $\tilde{\varphi}$的意义在于$\mathrm{St}(\tilde{\varphi})$等价于$\varphi^{*}$；
  \item 公式(\ref{form_defpar2})的意义在于使得谓词$\gamma$恒为真，则$\gamma^{*}$恒为假，即满足$\gamma^{*}<\gamma$；
  \item 公式(\ref{form_defpar3})对应$\mathrm{St}(\gamma\rightarrow Q')~=~(\gamma\rightarrow Q')\wedge(\gamma^*\rightarrow Q'^*)$，这使所有的$Q'$ 恒为真（$\gamma$ 为真），其中$Q'\in\sigma'_v$，意义在于让所有谓词$Q'$对应的$Q'^{*}$ 能够任意变化。
\end{itemize}
...
\begin{align*}
    \mathbf{CIRC}[\theta;P_1,P_2,P_3;Z]~=~\exists Q_Z\exists\gamma\mathbf{SM}[\psi;P_1,P_2,P_3,Q_Z,\gamma]
\end{align*}
\end{example}

从并行限定理论翻译定义$Tr_v$来看，对于每一个可变的谓词都引入了对应的一个新的辅助谓词，这个谓词数量的增加是可以接受的。
