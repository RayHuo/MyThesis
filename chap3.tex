...
在引入了$EC_U(P)$和$ECC_U(P,X)$后，依据命题\ref{satisfy_proposition}，可以知道原程序的模型可以通过$bottom$和$top$两部分的模型计算得到。然而，直接使用$P \backslash b_U(P)$作为$top$的话并不能求解出原程序剩下的回答集。这是因为，
接下来，本文引入逻辑程序中的环与环公式的概念，来定义可以用于求解原程序的$top$部分。首先，这里定义两个结构性的规则集合。
