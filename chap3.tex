...
程序分割方法涉及多个规则或原子集合的计算，可以把这些相关的规则或原子集合分为底部和顶部两部分进行分析，可以得到以下的结果：
...
可以看到这些规则或原子集合除了$DSL_U(P,X)$外，都可以在线性时间内得到，故这部分的耗时基本不会对整体计算造成影响。而对于$DSL_U(P,X)$的时间复杂度则主要受到$C=|head(in_U(P)) \cup body^+(out_U(P))|$的影响，而这部分事实上可以通过有效选择$U$的内容而使$O(M*2^C)$维持在一个较小的数值下。直观来说就是可以通过找出合适的$U$来使得被破坏且环公式不能被底部回答集$X$所满足的环尽量地少。
